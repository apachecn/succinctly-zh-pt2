...
在本章中，我们将创建一个外接程序模块来探索当前在 IDE 中打开的解决方案，并呈现关于该解决方案的一些关键统计信息的摘要屏幕。它提供了一个如何以编程方式访问解决方案各个方面的基本示例。

## 解决方案信息向导

使用向导和以下设置启动解决方案信息加载项:
...
| ![](../Images/image017.png) | 注意:请务必添加对系统的引用。项目的窗体。 |
...
项目对象的 **Kind** 属性是一个 GUID 字符串，所以我们在确定项目类型时需要一点帮助。为此，我们需要在外接程序项目中添加对 **VSLangProj** 的引用。
...
项目 GUIDS 存储在注册表项 HKLM \软件\微软\VisualStudio\ <vers>项目中。如果在**解决方案**加载项期间需要报告其他项目类型，您可以将 GUID 复制到其他常量中。以下是 VS 项目的一些示例常数:</vers>
...
您可以下载项目代码或者创建类似于图 12 中的表单。外接程序代码假定文本框控件名为 SOLINFO。请确保名称相同，并将其修饰符标记为 PUBLIC，以便外接程序可以将解决方案信息放在表单上。
