...
| ![](../Images/note.png) | 注意:微软 Azure 不支持 VHDX 格式。您可以使用 Hyper-V 管理器或转换-VHD PowerShell cmdlet 将磁盘转换为 VHD 格式。 |
...
| ![](../Images/note.png) | 注意:在本节中，我们将只讨论与在 Azure 中提供 SQL Server 映像相关的 PowerShell cmdlets。Azure PowerShell cmdlets 的完整列表及其定义可在以下链接中找到:[https://msdn.microsoft.com/library/azure/jj554330.aspx](https://msdn.microsoft.com/library/azure/jj554330.aspx) |
