...
| ![](../Images/note.png) | 注意:RDD 是延迟评估的数据结构。简而言之，这意味着没有与在 RDDs 上调用转换相关的处理。 |
...
| ![](../Images/note.png) | 注意:调用操作会触发计算操作。动作=计算 |
...
| ![](../Images/tip.png) | 提示:Spark 需要内核和 RAM 大多数情况下，建议避开磁盘。 |
...
| ![](../Images/note.png) | 注意:排序是一项昂贵的操作。尝试对较小的集合进行排序。 |
