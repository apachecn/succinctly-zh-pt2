...
| ![](../Images/note.png) | 注意:那个？在 INT 和 DOUBLE 类型声明之后，允许我们在这些字段中放置一个 NULL 值，如果在消息文本中找不到匹配项，这更合适。是`Nullable` < `int` >或`Nullable` < `double` >的捷径。 |
...
| ![](../Images/note.png) | 注意:在两次检查后，仍然有可能有一个有效但未发行的信用卡号，但是这样的功能需要与信用卡发行者进行某种 API 通信。 |
...
| ![](../Images/note.png) | 注意:上面的示例电子邮件正则表达式模式在中不起作用。NET，只有 Perl，但是说明了验证电子邮件的模式会变得多么复杂。 |
