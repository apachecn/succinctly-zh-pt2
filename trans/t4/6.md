# 第 6 章使用 SQL Server 元数据

## 微软。SMO

SMO 代表服务器管理对象。SMO 是一个对象集合，旨在为您提供管理 SQL server 所有方面的编程访问。我们不需要所有这些功能，但是该库还提供了一种简单直接的方法来访问 SQL Server 的数据字典。

| ![](../Images/note.png) | 注意:使用 SMO 将直接把你绑在服务器上。如果您使用的是不同的 RDMS，您仍然可以通过直接查询数据库数据字典中的表来检索这些元数据。请参考您的 RDMS 文档。 |
...
![http://merc.tv/img/fig/Northwind_diagram.jpg](../Images/image016.jpg)
