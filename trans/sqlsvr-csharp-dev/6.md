...
| ![](../images/00011.gif) | 提示:缓慢执行查询的问题是，有时它们运行得非常慢(比如一个小时或更长时间)。在这种情况下，“包含实际执行计划”不是一个选项，因为它需要您运行查询，但您不能等待几个小时才能完成，这会占用资源并可能阻塞其他查询。对于这种情况，SQL Server 在上下文菜单中有一个选项“显示预计执行计划”此选项不运行查询，仅创建估计计划。它可能不会给你实际的计划，但通常它相当准确，你至少可以检查计划并找出可能的瓶颈。 |
