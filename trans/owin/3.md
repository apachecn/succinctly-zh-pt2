...
![New ASP.NET Project](../Images/image031.png)

图 28:新 ASP.NET 项目

| ![](../Images/note.png) | 注意:您也可以使用网络应用编程接口模板创建项目，但这也将包括对 ASP.NET MVC(依赖于系统)的引用。用于提供应用编程接口帮助页面。总的来说，你不需要它，尤其是在 OWIN 治下。它不会工作，因为 ASP.NET MVC 还不兼容 OWIN。 |
...
![Nuget Package Manager with Microsoft.AspNet.WebApi.Owin selected](../Images/image033.png)
...
| ![](../Images/tip.png) | 提示:如果你想用一个没有 JSON 可视化工具的浏览器来测试你的 Web API，你可以去掉`JsonFormatter`，默认设置`XmlSerializer`总是返回结果的 XML 表示，就像前面的代码清单一样。 |
...
![Web API Response](../Images/image035.png)
...
![SignalR Self Host package](../Images/image036.png)
...
![Project folder structure](../Images/image038.png)

图 33:项目文件夹结构

这是一个控制台应用程序，因此默认情况下静态文件不会复制到输出文件夹。记得将**构建动作**指定为**内容**，将**复制到输出目录**指定为**始终复制**。

![Build action](../Images/image039.png)
...
![Directory browsing](../Images/image040.png)
