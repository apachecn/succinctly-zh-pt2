...
| ![](../Images/tip.png) | 提示:在发布应用程序之前，记得用 CurrentApp 类替换 CurrentAppSimulator 引用；否则，您的代码将无法工作。或者，您可以使用条件编译在调试模式下使用当前的模拟器类，在发布模式下使用当前的模拟器类。 |

| ![](../Images/note.png) | 注意:如果您尝试使用之前在 Windows Phone 上设置 ExpirationDate 的示例代码，您会注意到该代码可以正常工作。平台确实支持 API，但是由于提交过程不支持这种情况，您将无法在发布的应用程序中使用它。 |
