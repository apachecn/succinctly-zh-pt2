...
| ![](../Images/note.png) | 注意:由于`WillExecute`没有接收到上下文，所以很难使用。大多数时候我们只是返回`true`。 |
| ![](../Images/note.png) | 注意:调用处理程序和行为之间的区别在于行为拦截类中所有可拦截的方法，而调用处理程序只拦截一个。 |
...
| ![](../Images/note.png) | 注意:拦截属性可以应用于密钥或组件类型。 |
...
| ![](../Images/note.png) | 注意:可以添加几种拦截行为；所有这些都将按顺序应用。 |
...
| ![](../Images/tip.png) | 提示:如果调用处理程序需要特殊配置(如设置其执行顺序，或者如果它没有公共的无参数构造函数)，您应该添加一个调用处理程序实例，而不是它的类型。 |
...
| ![](../Images/tip.png) | 提示:只有当所有规则都匹配时，才会应用调用处理程序。 |
| ![](../Images/tip.png) | 提示:您可以添加任意数量的呼叫处理程序；它们将按照[顺序](http://msdn.microsoft.com/en-us/library/microsoft.practices.unity.interceptionextension.icallhandler.order.aspx)属性进行处理和排序。 |
...
| ![](../Images/note.png) | 注意:所有取名字的规则都接受正则表达式。 |
...
| ![](../Images/note.png) | 注意:其中一些调用处理程序需要特定的安装过程。不要忘记阅读他们的文档。 |