...
因此，让我们在配置文件中声明一个Unity配置部分，并添加一个Unity条目:
...
该文件包含Unity配置元素的 [XML 模式定义(XSD)](http://en.wikipedia.org/wiki/XML_Schema_%28W3C%29) 。完成此步骤后，您现在在配置文件的 Unity 部分中拥有了 IntelliSense:
...
可能会出现这样的情况:您想要解析的组件的参数(构造函数或方法参数、属性值)与注册该组件的参数不同。我不是说你应该这样做，因为这需要知道组件是如何注册的，这在某种程度上违背了 IoC 的宗旨。但是，无论如何，Unity允许你这样做。
...
| ![](img/note.png) | 注意:如果你想从公共服务定位器回到Unity，你只需要检索未命名的`IUnityContainer`实例。 |
