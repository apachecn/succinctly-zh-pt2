...
| ![](../Images/tip.png) | 提示:别忘了 Dispose 只针对部分终身经理人。 |
...
| ![](../Images/note.png) | 注意:记住注册是由一个基本类型(类或接口)和一个名称组成的。同名类型的每次后续注册(或缺少同名类型)都会覆盖上一次注册。 |
| ![](../Images/tip.png) | 提示:默认情况下，只有带有公共、无参数构造函数的类型才能与`RegisterType`一起使用。稍后，我们将看到如何通过让 Unity 自动注入依赖项来绕过这个限制。 |
...
| ![](../Images/tip.png) | 提示:`RegisterInstance`注册的对象为单例，`RegisterType`注册的对象每次被请求时默认创建。 |
| ![](../Images/note.png) | 注意:Unity 总是在`IUnityContainer`键下自动注册，没有名字。 |
| ![](../Images/note.png) | 注意:尽量将所有注册保存在同一个地方(bootstrap 方法)，因为这样更容易找到和更改。 |
...
| ![](../Images/note.png) | 注意:如果使用 Visual Studio 2012，可以从[微软企业库下载中心](http://www.microsoft.com/en-us/download/details.aspx?id=38789)下载企业库配置控制台。这有助于您通过用户界面配置应用程序配置文件中的所有企业库模块。 |
...
| ![](../Images/tip.png) | 提示:在您的 XML 配置中一定要使用完全程序集限定的类型名称，以便 Unity 可以找到这些类型。 |
...
| ![](../Images/tip.png) | 提示:别忘了调用`LoadConfiguration()`否则你的 XML 注册就找不到了。 |
...
| ![](../Images/tip.png) | 提示:如果您试图获取的注册不存在，Unity 将抛出一个异常。 |
...
| ![](../Images/tip.png) | 提示:`ResolveAll`只返回已命名的注册。 |

| ![](../Images/note.png) | 注意:无论注册方法如何，Unity 都会解析类型。 |
...
| ![](../Images/note.png) | 注意:尽量避免使用分辨率覆盖，因为这不是一个好主意。 |
...
| ![](../Images/tip.png) | 提示:你不需要用`Lazy<T>`或`Func<T>`注册类型，Unity 为你做。 |
...
| ![](../Images/note.png) | 注意:Singleton 和 transient 是类型别名。请参见类型别名。 |
...
| ![](../Images/tip.png) | 提示:唯一可以传递给`RegisterInstance`的有意义的终身经理是`ContainerControlledLifetimeManager`(默认)或`ExternallyControlledLifetimeManager`。两者的区别在于，后者在容器被处置时不会处置已注册的实例。 |
| ![](../Images/tip.png) | 提示:要使用 PerRequestLifetimeManager，您需要添加对 [Unity 的引用。Mvc](http://www.nuget.org/packages/Unity.Mvc) NuGet 包。 |
...
| ![](../Images/tip.png) | 提示:如果不使用别名，请确保使用生存期管理器类的程序集限定名。 |
...
| ![](../Images/tip.png) | 提示:生存期管理器实例不能被不同的注册共享，如果您尝试的话，`Register*`方法会抛出一个异常。 |
...
| ![](../Images/tip.png) | 提示:别忘了`Dispose`只是给一些终身经理人打的电话。 |
...
| ![](../Images/note.png) | 注意:如果你想从公共服务定位器回到统一，你只需要检索未命名的`IUnityContainer`实例。 |
...
| ![](../Images/note.png) | 注意:有些人认为服务定位器模式实际上是一件坏事。我理解这些论点，但不太同意。关于这方面的深入报道，请阅读马克·塞曼的博文[“服务定位器是一种反模式”](http://blog.ploeh.dk/2010/02/03/ServiceLocatorisanAnti-Pattern/)。 |