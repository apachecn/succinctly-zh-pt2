...
| ![](../Images/tip.png) | 提示:您需要添加对系统的引用。配置程序集以及 NuGet 包企业库。策略注入。请参见面向方面编程。 |
...
| ![](../Images/tip.png) | 提示:还是那句话，别忘了打`LoadConfiguration()`否则分机不会加载。 |
...
| ![](../Images/tip.png) | 提示:您需要添加对程序集 system . runtime . cache 的引用 |
...
| ![](../Images/note.png) | 注意:在现实情况中，您可能希望将`ICallHandler`实现与`HandlerAttribute`分离，以获得更好的解耦和可管理性。 |
