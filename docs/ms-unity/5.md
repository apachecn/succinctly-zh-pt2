...
4.  [IDisposable。处置](http://msdn.microsoft.com/en-us/library/system.idisposable.dispose.aspx):仅在Unity容器被处置时调用。有责任处置由生命周期管理器创建的所有实现 [IDisposable](http://msdn.microsoft.com/en-us/library/System.IDisposable.aspx) 的实例。
...
我认为这很容易理解:应该有一个`LoggerFactory`类公开一个名为`Create`的静态方法，该方法返回一个`ILogger`实例。当Unity解决`ILogger`类型的实例时，将使用此方法构建。
