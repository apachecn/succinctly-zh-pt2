...
| ![](../Images/tip.png) | 提示:为了编译这个例子，您需要引用 NHibernate 命名空间。 |
...
| ![](../Images/note.png) | 注意:创建会话工厂后，不要更改配置实例。 |
...
| ![](../Images/tip.png) | 提示:每当任何会话操作引发异常时，会话实例都会变得不可用，并且必须被处置。 |
...
| ![](../Images/tip.png) | 提示:导入 NHibernate，NHibernate。临朐，和 。 |
...
| ![](../Images/note.png) | 注意:不要因为常量的存在而惊慌；所有常量都将被转换为参数，以便重用查询的执行计划。这也阻止了 SQL 注入。 |
...
| ![](../Images/tip.png) | 提示:要尝试这些示例，请导入 NHibernate 命名空间。 |
...
| ![](../Images/tip.png) | 提示:导入命名空间 NHibernate.Transform。 |
...
| ![](../Images/tip.png) | 提示:导入命名空间 NHibernate 和 NHibernate.Criterion。 |
...
| ![](../Images/tip.png) | 提示:导入 NHibernate。标准命名空间。 |
...
| ![](../Images/note.png) | 注意:NHibernate 会对您的查询进行适当的更改，例如将其包装在另一个进行分页的查询中。 |
...
| ![](../Images/tip.png) | 提示:小心！如果您试图在不支持多查询的数据库服务器上创建多查询，NHibernate 将引发异常。 |
...
| ![](../Images/tip.png) | 提示:如果给定的数据库引擎不支持未来，它将静默地忽略未来的调用，而是立即执行查询。这是未来查询的一大优势。 |
...
| ![](../Images/tip.png) | 提示:要让惰性加载工作，它的包含实体和属性定义都必须声明为惰性——顺便说一下，这是默认的——并且必须实现为自动(意味着没有支持字段)虚拟属性。不能密封包含类。 |
...
| ![](../Images/note.png) | 注意:惰性属性、关联或集合只会在被访问时从数据库中提取一次。之后，它将存储在内存中。如果一个实体有几个惰性属性，而不是关联或集合，那么它们都会被同时加载。这与关联和集合不同。 |
...
| ![](../Images/tip.png) | 提示:仅当不使用延迟加载时，将提取策略指定为连接才有意义。此外，禁用延迟加载和使用选择来获取也是没有意义的。 |
...
| ![](../Images/note.png) | 注意:如果会话的缓存中已经存在指向同一记录的实例，将返回该实例而不是代理。 |
...
| ![](../Images/note.png) | 注意:如果您不使用惰性加载，您不需要将属性和方法标记为虚拟的，并且您可以有密封的类。 |
...
| ![](../Images/tip.png) | 提示:类伪属性只能在查询类层次结构时使用；否则，它是无用的，并且会导致错误。 |
...
| ![](../Images/tip.png) | 提示:正如您现在应该知道的，您不需要绑定到任何特定的应用编程接口，并且可以自由使用您喜欢的任何东西，只要它最适合您的需求。 |******