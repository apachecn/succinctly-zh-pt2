...
![../../../Desktop/eventloop.png](../images/00009.jpeg)
...
| ![](../images/00005.gif) | 注意:回调函数的第一个参数是可能的错误，这是一个公认的模式。 |
...
| ![](../images/00005.gif) | 注意:setImmediate 和 process.nextTick 函数有些混乱。它们看起来相似(而且确实如此)，但它们有一个微小的细微差别。setImmediate 将函数放在队列中，因此如果队列中有其他函数，setImmediate 函数将在这些其他函数之后执行。process.nextTick 将该函数放在队列的最前面，这样它将恰好在下一个 Tick 执行，绕过队列中的其他函数。 |
