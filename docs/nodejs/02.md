...
为了编写异步应用，我们需要支持回调模式的工具的支持。在Node标准库中，有`EventEmitter`模块，它公开了实现观察者的功能。
...
这是异步流的天真实现，但重要的是理解并对这些模式充满信心，因为作为Node开发人员，我们必须非常了解事件循环是如何工作的，以及我们如何管理异步调用。
