...
这个方程比它最初出现时要简单。大写 *C* 是一个整体聚类。小写的 *m* 是集群的数量。小写的 *k* 是一个从零开始的聚类指数。大写 *P* 表示“的概率。”大写 *A* 表示属性(如颜色)。大写 *V* 表示属性值(如红色)。
...
在定义了一种度量聚类良好性的方法之后，聚类分类数据的第二个挑战性步骤是提出一种搜索所有可能的聚类的技术。一般来说，检查数据集的每个可能的聚类是不可行的。例如，即使是只有 100 个元组的数据集，且 *m* = 2 个聚类，也有 2 个 <sup>100</sup> / 2！= 2 <sup>99</sup> = 633，825，300，114，114，700，748，351，602，688 个可能的集群。即使你能以某种方式每秒检查一万亿个集群，也需要大约 190 亿年才能全部检查完。相比之下，宇宙的年龄估计约为 140 亿年。

GACUC 算法使用所谓的贪婪凝聚方法。这个想法是从用单个数据元组播种每个集群开始的。然后，对于每个剩余的元组，确定如果当前元组被添加到其中，哪个聚类将产生最佳的整体聚类。那么给出最佳 CU 的元组实际上被分配给该簇。
...
事实证明，为每个集群选择一个初始数据元组并不容易。一种天真的方法是简单地选择 *m* 个随机元组作为种子。然而，如果种子元组彼此相似，那么得到的聚类可能很差。为每个聚类选择初始元组的更好方法是选择彼此尽可能不同的 *m* 元组。
...
所有的集群逻辑都包含在一个名为 CatClusterer 的程序定义类中。所有的程序逻辑都包含在 Main 方法中。Main 方法首先在一个数组样式矩阵中设置七个硬编码的、颜色大小较重的数据项:
...
int[]聚类= cc。集群(numRestarts，out Cu)；

ShowVector(聚类，真)；

控制台。WriteLine("最终 CU 值=+CU。ToString(" F4 ")；

实例化 CatClusterer 对象，并调用其 Cluster 方法。在幕后，方法集群调用方法集群一次几次(`numRestarts`)，跟踪找到的最佳集群。返回最佳聚类及其相关联的 CU 值。

在演示程序中，最终的最佳聚类被存储到一个名为`clustering`的数组中，并被编码为{ 0，1，0，0，1，1，0 }。这意味着数据元组 0 被分配给集群 0，数据元组 1 被分配给集群 1，数据元组 2 被分配给集群 0，依此类推。找到的最佳聚类的最终 CU 值存储在外参数`cu`中，为 0.3299。

演示程序最后调用助手方法 ShowClustering 来显示原始数据，按集群排列:
...
GACUC 算法多次计算类别效用。每次都可以从头开始计算 CU，这将涉及扫描整个数据集并计算分配给每个集群的属性值的数量。但是更有效的方法是将每个属性值的当前计数存储在数据结构中，然后在每个数据元组被分配给一个集群时更新数据结构。数据结构`valueCounts`存储该信息。
...
类 CatClusterer 有六个私有数据成员，如图****2-b**所示。对于包括我在内的大多数开发人员来说，在编写机器学习代码时，拥有关键数据结构的图表是必不可少的。类成员`rnd`用于生成要分配给集群的初始元组的候选集，以及以随机顺序遍历剩余元组。**

 **该类只公开了两个公共方法:构造函数和聚类方法。助手方法 ClusterOnce 执行一遍 GACUC 算法，返回找到的聚类和关联的 CU 作为输出参数。方法集群调用集群一次`numRestart`次，返回找到的最佳集群和 CU。
...
做大部分工作的方法 ClusterOnce 调用助手方法 GetGoodIndices、Assign、Unassign、Shuffle 和 MaxIndex。方法 GetGoodIndices 生成彼此不同的初始数据元组。分配更新所有数据结构，以将元组分配给集群。取消分配反转“分配”操作。方法 Shuffle 用于以随机顺序呈现数据元组。方法使用 MaxIndex 来寻找最佳的建议群集分配。
...
方法集群出现在**清单 2-c** 中。请注意，该方法不接受表示要聚集的数据的参数；假设数据作为类成员可用。
...
方法集群本质上是方法集群的包装器。请注意，传递给方法集群一次的随机化种子值是当前迭代变量`start`的值。当使用重启算法时，这个技巧是一种常见的设计模式，这样工作方法就不会在每次迭代中返回相同的结果。
...
助手方法 Initialize 执行三个任务。首先，`clustering`数组中的值都设置为-1。这允许算法知道数据元组是否已经被分配给集群。其次，`clusterCounts`中的值被设置为 0 以重置数组，该数组保存对集群一次的任何先前调用的计数。第三，数据结构`valueCounts`中的值设置为 0。

接下来，方法 ClusterOnce 选择第一个元组，并将它们分配给集群:
...
方法 GetGoodIndices 返回`numClusters`数据元组不同的数据索引。如前所述，通常不可能检查初始元组的所有可能的候选集合，因此检查集合的`numTrials`。在找到这些好的索引(数据元组不同)之后，它们相关的数据元组被分配给集群。

一个简短的例子将有助于澄清。对于演示数据，七个数据元组和集群数设置为三，方法 GetGoodIndices 可能返回{ 6，0，1 }。这些是由汉明距离定义的彼此非常不同的三个数据项的索引:
...
for(int k = 0；k < numClusters++(k)//每个可能的集群
...
Assign(idx，bestK)；//现在我们知道哪个集群的 CU 最好了

} //每个元组

此时，所有数据元组都已分配给一个集群。方法集群一次计算最终的类别实用程序，并将集群作为显式返回值返回，将 CU 作为输出参数返回:
...
在集群的数量非常大并且属性的数量也很大的情况下，您可以修改 GetGoodIndices 来只检查候选元组的相邻对。本章末尾的程序清单为此提供了示例代码。
