...
根据《牛津英语词典》，它被定义为看到某样东西的能力，或者从某个特定的地方被看到的能力，当在网络环境中使用时，比如Nancy，或者实际上任何产生视图的框架作品，这都非常符合它的含义。
...
那么，为什么有必要定义一个观点，而不是仅仅提到它是Nancy产生的？正如您到目前为止所看到的，Nancy 是异常模块化的，到目前为止，您可以将不同的视图引擎插入到框架中应该不足为奇。
...
图 10:当你要求Nancy展示一些不存在的东西时，她是这么做的
