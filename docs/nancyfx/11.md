...
在这一章中，我们开始进入好的方面:我们即将打开发动机罩(或引擎盖，取决于你住在哪里)，并开始调整和修补Nancy的内部。

什么是Bootstrap？它与每个人用来制作漂亮的基于网络的用户界面的客户端框架无关。根据字典，这是它的定义:

![](img/image030.jpg)

图 28:Bootstrap的字典定义(Dictionary 提供)
...
这有点像买一辆二手车，然后调整发动机以提高性能，添加新的内饰和新的收音机，并提供其他改进的功能以在车辆上标记您自己的身份。通过编写和使用自定义引导程序，你可以让Nancy做各种现成的额外工作。
...
Nancy在两个方面使用Bootstrap程序。它将它作为一种方法向开发人员展示——所有的钩子、配置选项和其他用户级的东西，这些都可以被使用框架的开发人员覆盖。
...
您会注意到所有对`/`的请求都被设置为在名为**页面**的文件夹中查找；您在这里不能做的一件事是将`/`指向您的应用根；Nancy认识到这带来的安全风险，不会允许它发生。

这实际上意味着我可以将正常的、静态的 HTML 文件放在 pages 文件夹中，然后添加 **/blah.html** 进行请求。Nancy将从那个文件夹中返回一个名为**blah.html**的静态 HTML 文件。这允许您混合传统的经典 HTML、脚本和 CSS 文件，这些文件无需Nancy处理，同时Nancy强大的路由引擎和应用编程接口/网址功能。我使用的方法正是我在[丽都网站](http://www.lidnug.org/)上所做的工作，在那里我使用 JavaScript 在客户端上直接高速地交付用户界面。然后，这个用户界面中的 JavaScript 在 Nancy 的 web 框架提供的不同端点上调用，为客户端发出的 AJAX 请求提供服务。
