...
在您的 Nancy 应用程序中任何可以访问`NancyContext`的地方，您都可以读取一个名为`CurrentUser`的属性。
...
打开你的 NuGet 包管理器，搜索并安装 Nancy.Authentication.Forms .一旦你安装了这个，在你的应用程序的根目录下创建一个名为`CustomBootstrapper.cs`的新类，并确保它有以下内容(记得根据你的项目需要调整名称空间之类的东西):
